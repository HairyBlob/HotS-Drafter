...
#        with tf.device('/gpu:0'):
    ...
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=conv_net_siamese(x, weights, biases, keep_prob), labels=y)
                                                                  + 0.0001*tf.nn.l2_loss(weights['heroes_w'])
                                                                  + 0.0001*tf.nn.l2_loss(weights['heroes_w2'])
                                                                  + 0.0001*tf.nn.l2_loss(weights['heroes_w3'])
                                                                  + 0.0001*tf.nn.l2_loss(weights['heroes_w4'])
                                                                  + 0.0001*tf.nn.l2_loss(weights['out']))
...
#        saver.restore(sess, os.path.join(dirname, 'discriminator_graph/discriminator'))
...
#            with tf.device('/gpu:0'):
            ...
            if step % (1000000 / batch_size) == 0:
                #Now, save the graph
                saver.save(sess, os.path.join(dirname, 'discriminator_graph\\discriminator_checkpoint'), global_step=0)
